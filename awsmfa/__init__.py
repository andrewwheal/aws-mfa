...
def validate(args, config):
    ...
    if should_refresh:
        get_credentials(short_term_name, long_term_name, key_id, access_key, args, config)


def get_credentials(short_term_name, long_term_name, lt_key_id, lt_access_key, args, config):
    if args.token:
